...
def create_blog(title, company_context=COMPANY_CONTEXT, content_type="blog"):
    ...
    try:
        ...
        end_time = time.time()
        print(f"  ⏱️  Request completado en {end_time - start_time:.1f} segundos")
        
        response.raise_for_status()
        return response.json()
    except requests.exceptions.Timeout:
        print(f"  ⏰ Timeout después de 5 minutos - la API está tardando mucho")
        return None
...
def main():
    ...
    print(f"⚠️  Nota: Cada blog puede tardar varios minutos debido al proceso de Writer/Editor")
    print("-" * 60)
    ...
    for i, title in enumerate(BLOG_TITLES, 1):
        print(f"\n[{i}/{len(BLOG_TITLES)}] Creando: {title}")
        blog_start_time = time.time()
        
        result = create_blog(title)
        
        blog_end_time = time.time()
        blog_duration = blog_end_time - blog_start_time
        
        if result:
            created_blogs.append(result)
            print(f"  ✅ Blog creado exitosamente en {blog_duration:.1f}s - ID: {result.get('id', 'N/A')}")
        else:
            failed_blogs.append(title)
            print(f"  ❌ Falló la creación del blog después de {blog_duration:.1f}s")
        
        # Pausa más corta ya que cada request ya tarda mucho
        if i < len(BLOG_TITLES):
            print("  ⏳ Esperando 1 segundo antes del siguiente...")
            time.sleep(1)
    ...
    # Resumen final
    print(f"\n📊 RESUMEN FINAL:")
    print(f"⏱️  Tiempo total: {total_duration/60:.1f} minutos")
    print(f"✅ Blogs creados exitosamente: {len(created_blogs)}")
    print(f"❌ Blogs que fallaron: {len(failed_blogs)}")
    print(f"📈 Tasa de éxito: {len(created_blogs)/(len(created_blogs)+len(failed_blogs))*100:.1f}%")
